...
async function fetchSettings() {
  try {
    ...
    const defaults = {
      ...
      contactFormAction: process.env.CONTACT_FORM_ACTION_DEFAULT || 'whatsapp', // Default from .env or hardcoded
      schoolContactEmail: process.env.SCHOOL_CONTACT_EMAIL_TO || '', // Default from .env
      adminSchoolWhatsappNumber: process.env.SCHOOL_WHATSAPP_NUMBER || ''// Display for admin, actual send uses .env for security
    };
    ...
    }return completeSettingscatch (error) {
    console.error('Error fetching settings:', error);
    const defaultsOnError = {
      // ... (your existing defaultsOnError, ensure to add new ones here too) ...
      contactFormAction: process.env.CONTACT_FORM_ACTION_DEFAULT || 'whatsapp',
      schoolContactEmail: process.env.SCHOOL_CONTACT_EMAIL_TO || '',
      adminSchoolWhatsappNumber: process.env.SCHOOL_WHATSAPP_NUMBER || '',
    };
...
async function saveSettingsToServer(settingsToSave) {
  try {
    ...
    if (!response.ok) {
        const responseText = await response.text();
        console.error(`SAVE SETTINGS FAILED: Server responded with ${response.status} to /api/settings. Body:\n`, responseText);
        ...
        try {
            if (response.headers.get("content-type")?.includes("application/json")) {
                ...
                errorDetail = errorJson.message || JSON.stringify(errorJson);
                 ...
                 errorDetail = `Received non-JSON response (HTML?) when saving settings. Check console for full HTML.`;
            }
        } catch (e) {
             errorDetail = `Received non-JSON response when saving settings and failed to parse it. Check console for full HTML.`;
        }
    ...
    console.error('Error in saveSettingsToServer function:', error);
...
async function saveAdminSettings() {
  ...
  gradientSections.forEach(section => {
      ...
      } else if (section === 'hero' && currentSettings.heroGradient) { // Special case for hero if not editable
...
function applyMapPublic(mapEmbedURL) {
    ...
    if (schoolMapIframe && mapContainer && mapPlaceholder) {
        if (mapEmbedURL && mapEmbedURL.trim() !== '') {
            ...
            // mapContainer.style.alignContent=''
...
function populateCarouselPublic(images, settings) {
    ...
    if (!images || images.length === 0) {
        ...
        // itemDiv.innerHTML = `${imgHtml}<div class="carousel-caption d-none d-md-block"><h5>${defaultAltText}</h5></div>`; // Optional caption
    ...
    images.forEach((image, index) => {
        ...
        itemDiv.innerHTML = imgHtml;
        // itemDiv.innerHTML = `${imgHtml}<div class="carousel-caption d-none d-md-block"><h5>${image.alt_text || 'School Image'}</h5></div>`; // Optional caption
        carouselInner.appendChild(itemDiv);
