...
async function fetchSettings() {
  try {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Network response was not ok (${response.status}) when fetching settings. Check console for HTML response.`);
    }
    const serverSettings = await response.json();
    
    // Define defaults for all settings, including new ones
    const defaults = {
      ...
      // New contact form defaults
      contactFormAction: 'whatsapp', // Default to WhatsApp if not set by server or .env
      schoolContactEmail: '', // Default empty, can be overridden by .env or admin
      adminSchoolWhatsappNumber: '' // Default empty, can be overridden by .env or admin
    };
    ...
    const completeSettings = { 
        ...
        // contactFormAction, schoolContactEmail, adminSchoolWhatsappNumber will be directly merged
    };
...
async function saveSettingsToServer(settingsToSave) {
  try {
    console.log(`Attempting to save settings to ${API_BASE_URL}/settings`);
    const response = await fetch(`${API_BASE_URL}/settings`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        // If you have CSRF tokens or other auth headers, add them here
      },
    ...
    if (!response.ok) {
      // Check if the server is trying to redirect (e.g., to login page)
      // A 302 status code is a common indicator, but checkAuth might also use 401/403
      // and then the browser follows the redirect if it's a full page load,
      // but fetch won't automatically follow redirects unless 'redirect: "follow"' is set.
      // However, the Express redirect will still result in HTML being sent.

      const responseText = await response.text(); // Get response as text first

      if (response.status === 401 || response.status === 403 || (contentType && contentType.includes("text/html") && (responseText.includes("<title>Login") || responseText.includes("login.html")) ) ) {
        // It's highly likely an authentication issue / redirect to login
        console.error('SAVE SETTINGS FAILED: Authentication error or redirect to login page. Status:', response.status);
        alert('Your session may have expired. Please log in again to save settings.');
        window.location.href = '/login.html?sessionExpired=true&returnTo=' + encodeURIComponent(window.location.pathname); // Redirect to login
        throw new Error('Authentication failed. Redirecting to login.'); // Stop further processing
      } else {
        ...
        if (contentType && contentType.includes("application/json")) {
            ...
            errorDetail = `Received non-JSON, non-HTML response. Raw text: ${responseText.substring(0,200)}...`;
        }
    ...
    if (contentType && contentType.includes("application/json")) {
        ...
        console.error('SAVE SETTINGS WARNING: Server response was OK, but not JSON. Content-Type:', contentType);
        const responseText = await response.text();
        console.log('Raw response text:', responseText.substring(0, 500));
        // Depending on your server, an OK response without JSON might still be a success.
        // Or it could be an issue. For now, let's assume it's a success if OK and not error above.
    ...
    // Alert is shown by the code that throws the error now, or if fetch itself fails (network error)
    if (!error.message.includes('Authentication failed')) { // Avoid double alert for auth errors
...
async function fetchCarouselImages() {
  try {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Network response was not ok (${response.status}) when fetching carousel images. Check console for HTML.`);
    }
...
async function removeCarouselImageAdmin(id) {
  ...
  try {
    ...
    if (!response.ok) {
        const responseText = await response.text();
        console.error(`REMOVE CAROUSEL IMG FAILED: Server responded with ${response.status}. Body:\n`, responseText);
        let errorData;
        try {
            errorData = JSON.parse(responseText);
        } catch (e) {
            errorData = { message: `Failed to remove image (${response.status}). Received non-JSON response (HTML?). Check console.` };
        }
...
async function loadAdminData() {
  try {
    ...
    if (isAdminPage()) {
      ...
      const contactAction = settings.contactFormAction || 'whatsapp'; // Default if undefined
      ...
      if (whatsappRadio && emailRadio) {
          if (contactAction === 'email') {
              emailRadio.checked = true;
          } else {
              whatsappRadio.checked = true; // Default to WhatsApp if value is 'whatsapp' or invalid
          }
      }
      ...
      // --- End Load Contact Form Settings ---
...
function loadGradientSettings(settings) {
    ...
    sections.forEach(section => {
        const gradientData = settings[`${section}Gradient`] || {}; // Default to empty object if undefined
        if (getElement(`${section}GradientColor1`)) { // Check if admin controls exist for this section
...
async function handleImageUpload(inputId, previewId, currentUrlId, endpoint, formDataKey) {
    ...
    if (uploadInput && uploadInput.files[0]) {
        ...
        try {
            console.log(`Attempting to upload ${formDataKey} to ${API_BASE_URL}/${endpoint}`);
            const response = await fetch(`${API_BASE_URL}/${endpoint}`, { method: 'POST', body: formData });

            if (!response.ok) {
                const responseText = await response.text();
                console.error(`UPLOAD FAILED for ${formDataKey}: Server responded with ${response.status} to /${endpoint}. Body:\n`, responseText);
                ...
                try {
                    if (response.headers.get("content-type")?.includes("application/json")) {
                        ...
                        errorDetail = `Received non-JSON response (HTML?) for ${formDataKey} upload. Check console for full HTML.`;
                    }
                } catch (e) {
                     errorDetail = `Received non-JSON response for ${formDataKey} upload and failed to parse it. Check console for full HTML.`;
                }
                throw new Error(errorDetail);
            }
            ...
            if (result.url) {
                ...
                console.log(`${formDataKey} uploaded successfully: ${finalURL}`);
            } else {
                console.error('URL not found in upload response for', formDataKey, 'Response:', result);
                throw new Error(`URL not found in ${formDataKey} upload response.`);
            }
        } catch (error) {
            console.error(`Error during ${formDataKey} upload process:`, error);
            throw new Error(`Failed to upload ${formDataKey}: ${error.message || error}`);
        }
    }
    return finalURL;
}
...
async function saveAdminSettings() {
  ...
  try {
    ...
    if (getElement('academicsImageUpload')?.files[0]) {
      ...
      console.error('An error occurred during one of the image uploads:', uploadError);
  ...
  const settingsToSave = {
    ...
    defaultLogoURL: getElement('defaultLogoURL')?.value || '',
    aboutUsImageURL: finalAboutImageURL, 
    defaultAboutImageURL: getElement('defaultAboutImageURL')?.value || '',
    academicsImageURL: finalAcademicsImageURL, 
    defaultAcademicsImageURL: getElement('defaultAcademicsImageURL')?.value || '',
    ...
    contactFormAction: querySelector('input[name="contactFormAction"]:checked')?.value || 'whatsapp', // Default to 'whatsapp' if nothing selected
    ...
    // --- End Save Contact Form Settings ---
  };
  
  const gradientSections = ['hero', 'about', 'admissions', 'academics', 'facilities', 'contact'];
  const currentSettings = await fetchSettings(); // Fetch current to preserve ones not editable on admin page if needed

  gradientSections.forEach(section => {
      if (getElement(`${section}GradientColor1`)) { // If admin controls for this gradient exist
      ...
      } else if (section === 'hero' && currentSettings.heroGradient) { // Special case for hero if not editable but exists
        settingsToSave.heroGradient = currentSettings.heroGradient; 
      } else if (section === 'hero') { // Default hero gradient if no controls and no prior setting
  ...
  try {
    ...
    if (result && !result.error) {
      ...
      loadAdminData(); 
        ...
        // Alert is already handled by saveSettingsToServer
        console.error("saveSettingsToServer indicated an error in its response object:", result.error);
    }
  } catch (error) { 
    console.error('Error in saveAdminSettings (final save step):', error);
  }
}
...
async function uploadCarouselImage() {
    ...
    try {
        ...
        if (!response.ok) {
            const responseText = await response.text();
            console.error(`CAROUSEL UPLOAD FAILED: Server responded with ${response.status}. Body:\n`, responseText);
            let errorData;
            try {
                errorData = JSON.parse(responseText);
            } catch (e) {
                 errorData = { message: `Upload failed (${response.status}). Received non-JSON. Check console.`};
            }
...
function populateSocialLinksPublic(socialLinks = {}, whatsappNumber) {
    ...
    for (const [elementId, url] of Object.entries(linkMapping)) {
        ...
        if (linkElement) {
            if (url && url.trim() !== '') {
                ...
                linkElement.parentElement.style.display = 'inline-block'; // Assuming link is child of an li or div
            } else {
    ...
    if (whatsappLinkEl) {
        if (whatsappNumber && whatsappNumber.trim() !== '') {
            ...
            whatsappLinkEl.style.display = 'flex'; // Or 'inline-block' depending on layout
        } else {
...
async function logoutAdmin() {
    if (confirm('Are you sure you want to logout?')) {
        try {
            ...
            if (response.ok) {
                const data = await response.json().catch(() => ({ message: "Logout successful, but no JSON response." }));
                alert(data.message || 'Logout successful.');
                window.location.href = '/login.html';
            } else {
                const responseText = await response.text().catch(() => "Could not read error response.");
                console.error('Logout failed on server:', response.status, responseText);
