...
const session = require('express-session'); // express-session itself
...
const { createClient } = require('redis'); // For Vercel KV
const RedisStore = require('connect-redis').default; // REVISED: Keep .default if your connect-redis version requires it (e.g., v6). For v7+, it's usually not needed.
...
if (process.env.KV_URL) {
    ...
    // NEW: Detailed Redis client event logging
    ...
    redisClient.connect()
      .then(() => {
        console.log('Redis client successfully connected via connect() promise.');
        redisStore = new RedisStore({ client: redisClient });
        console.log('RedisStore initialized.');
      })
      .catch(err => {
        console.error('Redis client .connect() promise FAILED:', err);
        console.error('WARNING: Sessions will likely not work. Check KV_URL and Redis service.');
      });
} else {
    console.error("FATAL: KV_URL environment variable is not set! Redis session store cannot be configured.");
    // process.exit(1); // Optionally exit if sessions are critical
}
...
// 2. CORS (REVISED: Robust CORS Configuration)
const allowedOrigins = [];
if (process.env.FRONTEND_URL) {
    allowedOrigins.push(process.env.FRONTEND_URL); // e.g., https://your-frontend.vercel.app
}
if (process.env.NODE_ENV !== 'production') {
    allowedOrigins.push('http://localhost:5173'); // For local Vite dev
    allowedOrigins.push('http://localhost:3000'); // For local frontend dev on other ports
    // Add any other local frontend origins you use
}
...
app.use(cors({
    ...
    credentials: true // IMPORTANT: Allows cookies for cross-origin requests
}));

// 3. Session Configuration (REVISED)
if (redisStore) { // Only use session middleware if Redis store is configured
    app.use(session({
      store: redisStore,
      ...
      cookie: {
        ...
        sameSite: process.env.NODE_ENV === 'production' ? 'lax' : 'lax' // Consider 'lax' or 'none' (if 'none', secure must be true)
        // REVISED: Removed 'credentials: "include"' - this is a client-side fetch option, not for server cookie config.
      }
    }));
    console.log("Session middleware configured with RedisStore.");
} else {
    console.warn("Session middleware NOT configured due to RedisStore initialization issues.");
    // Fallback or error handling if sessions are absolutely critical
    // For example, you could use a memory store for local dev if Redis fails
    // but this would be problematic in serverless environments.
}


// 4. Static Files
app.use(express.static(path.join(__dirname, 'public')));
app.use('/uploads', express.static(path.join(__dirname, 'public', 'uploads'))); // Consider if this needs auth

// --- Database Table Initialization ---
async function initializeDatabase() {
    if (!db) {
        console.error("Database client not initialized. Skipping table initialization.");
        return;
    }
    try {
...
// --- Authentication Middleware (REVISED: Enhanced Logging) ---
function checkAuth(req, res, next) {
  // NEW: Log the entire session object for better debugging
...
// --- Authentication API Routes (REVISED: Enhanced Logging) ---
app.post('/login', async (req, res) => {
    ...
    if (!req.session) { // NEW: Check if session middleware is even working
        console.error("LOGIN ABORTED: req.session is not defined. Session middleware might not be configured or running correctly.");
    ...
    try {
        ...
        if (user) {
            ...
            if (match) {
                req.session.regenerate((regenErr) => { // Regenerate session to prevent fixation
                    if (regenErr) {
                    ...
                    // NEW: Log before save
                    console.log(`LOGIN: User '${user.username}' credentials valid. Session data set: authenticated=${req.session.authenticated}, username='${req.session.username}'. Session ID: ${req.sessionID}. Attempting to save...`);

                    req.session.save((saveErr) => { // Explicitly save the session
                        if (saveErr) {
                        ...
                        console.log(`LOGIN > Current req.session after save:`, JSON.stringify(req.session));
...
app.post('/api/logout', (req, res) => {
    if (req.session) {
        ...
        req.session.destroy(err => {
            ...
            res.clearCookie('connect.sid', { path: '/' }); // Ensure path matches cookie setting if specified
...
app.post('/api/settings', checkAuth, async (req, res) => { // checkAuth is applied here
    const { settings } = req.body;
    console.log(`/api/settings POST request by user: ${req.session.username}`); // Log who is making the change
...
app.delete('/api/carousel/:id', checkAuth, async (req, res) => {
    ...
    try {
        ...
        if (imageUrlToDelete && imageUrlToDelete.startsWith('http')) { // Only attempt delete if it's a URL
            try {
                // Check if blob exists before attempting to delete
                const blobHead = await head(imageUrlToDelete); // Verifies existence
                if (blobHead) { // Vercel Blob SDK's head doesn't throw 404 but returns null or object
                    await del(imageUrlToDelete);
                    console.log(`Successfully deleted from Vercel Blob: ${imageUrlToDelete}`);
                } else {
                     console.warn(`Blob not found on Vercel Blob (head returned falsy): ${imageUrlToDelete}`);
                }
            } catch (blobError) {
                // Handle specific error codes if head throws (e.g., @vercel/blob might change behavior)
                if (blobError.status === 404 || (blobError.message && blobError.message.includes('404'))) { // More robust 404 check
                    console.warn(`Blob not found on Vercel Blob (already deleted or invalid URL?): ${imageUrlToDelete}`);
                } else {
                    console.error(`Error interacting with Vercel Blob for ${imageUrlToDelete}:`, blobError);
                    // Decide if this should be a critical error for the transaction
                    // For now, we'll log it and proceed with DB commit if DB part was successful
                }
            }
        } else {
            console.log(`Skipping Vercel Blob deletion for ID ${imageId} as image_url is not a valid http(s) URL: ${imageUrlToDelete}`);
        }
        
        await client.query('COMMIT');
        console.log(`Carousel image ID ${imageId} DB record deleted. Blob deletion attempted/logged.`);
...
app.post('/api/submit-contact', async (req, res) => {
    // ... (Your existing contact form logic - seems fine, no session dependency here)
...
app.listen(SERVER_PORT, () => {
    ...
    if (!process.env.KV_URL) {
        console.error("CRITICAL WARNING: KV_URL is not set. Redis session store will not work.");
    }
     if (!process.env.POSTGRES_URL) {
        console.error("CRITICAL WARNING: POSTGRES_URL is not set. Database operations will fail.");
    }
});
